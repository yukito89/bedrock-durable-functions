...
#### 履歴ページでダウンロードできない
**原因**: `results`コンテナにファイルが存在しない、または削除された

**解決策**:
- 処理が完了しているか `/api/status/{instanceId}` で確認
- ライフサイクル管理で自動削除されていないか確認（1日以上経過）
...
#### 履歴ページが空
**原因**: まだ処理を実行していない、または結果が削除された

**解決策**: メインページでファイルをアップロードして処理を実行
